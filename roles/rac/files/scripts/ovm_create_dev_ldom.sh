...
L_ENV=`echo $ENV | tr -s '[:upper:]' '[:lower:]'`
U_ENV=`echo $ENV | tr -s '[:lower:]' '[:upper:]'`
...
echo     ldm add-vdisk id=2 gidisk_11.2.0.4 ${NAME}-gidisk0@primary-${VDS} $NAME
echo     ldm add-vdisk id=3 dbdisk_11.2.0.4 ${NAME}-dbdisk0@primary-${VDS} $NAME
...
echo     ldm add-vdisk id=15 ${L_ENV}_fra_01 ${U_ENV}_FRA_01@primary-${VDS} $NAME
echo     ldm add-vdisk id=16 ${L_ENV}_fra_02 ${U_ENV}_FRA_02@primary-${VDS} $NAME
echo     ldm add-vdisk id=17 ${L_ENV}_ocr_01 ${U_ENV}_OCR_01@primary-${VDS} $NAME
echo     ldm add-vdisk id=18 ${L_ENV}_ocr_02 ${U_ENV}_OCR_02@primary-${VDS} $NAME
echo     ldm add-vdisk id=19 ${L_ENV}_ocr_03 ${U_ENV}_OCR_03@primary-${VDS} $NAME
    echo "#Done."
fi
